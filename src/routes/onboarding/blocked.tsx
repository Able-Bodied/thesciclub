  ...
  return (
    <div className="mx-auto flex h-dvh w-full max-w-[480px] flex-col bg-canvas">
      <div className="flex-1 overflow-y-auto px-[18px] pt-12">
        ...
        <h1 className="mt-4 font-extrabold font-head text-[25px] text-ink leading-tight tracking-[-0.02em]">
          This number isn't on the list
        </h1>
        <p className="mt-2.5 text-[14.2px] text-ink2 leading-[1.52]">
        ...
        <div className="mt-4 rounded-[17px] border border-line bg-paper px-3.5">
          {VOUCHERS.map(([code, name, blurb]) => (
            <div key={code} className="flex gap-3 border-line border-b py-3.5 last:border-b-0">
              <span className="grid h-[30px] w-[30px] flex-none place-items-center rounded-[10px] bg-gradient-to-br from-gold-dp to-gold font-extrabold font-head text-[9.5px] text-white">
                {code}
              </span>
              <span>
                <span className="block font-extrabold font-head text-[14px]">{name}</span>
                <span className="mt-0.5 block text-[12.5px] text-grey leading-[1.42]">{blurb}</span>
              </span>
            </div>
          ))}
        </div>

        <p className="mt-3 text-[12.5px] text-grey leading-[1.5]">
      ...
      <footer className="flex-none px-[18px] pt-3 pb-[max(22px,env(safe-area-inset-bottom))]">
        <button
          ...
          className="flex min-h-[48px] w-full items-center justify-center rounded-[13px] bg-gold font-bold font-head text-[#2A1E06] text-[15px]"
        >
          Try another number
        </button>
      </footer>
